...
class GetAdversary(object):
        ...
        with tqdm(total=k) as pbar:
            if self.sampling_method == 'initial_seed' or self.sampling_method == 'training_data' or\
                ...
                if sampling_method == 'kcenter':
                    ...
                    elif second_sss == 'margin':
                        sss = MarginSelectionStrategy(k, Y_copy, previous_s=s1)
                    elif second_sss == 'uncertainty':
                        sss = UncertaintySelectionStrategy(k, Y_copy, previous_s=s1)
                    elif second_sss == 'graph_density':
                        sss = GraphDensitySelectionStrategy(k, Y_copy, self.get_initial_centers(), previous_s=s1)
                    elif second_sss == 'adversarial_deepfool':
                        sss = AdversarialDeepFoolStrategy(size=k, X=X_rest, Y_vec=Y_copy, copy_model=self.attack_model,
                                                          previous_s=s1)
                    elif second_sss == 'kcenter':
                        sss = KCenterGreedyApproach(k, Y_copy, self.get_initial_centers(), previous_s=s1)
                    elif second_sss == 'bayes_ald_dropout':
                ...
                chosed_idx = [list(self.idx_set)[int(e)] for e in s]
                self.idx_set = self.idx_set - set(chosed_idx)
                self.pre_idxset = np.append(self.pre_idxset, chosed_idx)
                print("self.pre_idxset:", self.pre_idxset)
                #Query
                ...
                for b in range(int(np.ceil(len(self.pre_idxset) / self.batch_size))):
                    ...
                    c_idx = self.pre_idxset[b * self.batch_size: min((1 + b) * self.batch_size, len(self.pre_idxset))]
                    ...
                    for m in range(len(c_idx)):
                        img_p_i = img_p[m].numpy() if isinstance(img_p[m], torch.Tensor) else img_p[m]
                        img_t_i = img_p_i.squeeze() if isinstance(img_p_i, np.ndarray) else img_p_i
                        self.transferset.append((img_t_i, y_b[m].squeeze()))
                    pbar.update(x_b.size(0))
    ...
    def get_transferset_shadow_test(self, k):
        ...
        target_shadow = []
        with tqdm(total=k) as pbar:
            shadow_idx_set = set(range(cfg.start_shadow_test, cfg.start_shadow_test+int(k/2)))
            shadow_idx_set_out = set(range(cfg.start_shadow_test_out, cfg.start_shadow_test_out+int(k/2)))
            for t, B in enumerate(range(start_B, end_B, self.batch_size)):
...
def Parser():
    ...
    parser.add_argument("-num_iter", default=cfg.num_iter, help="num of iterations")
    ...
    parser.add_argument('-num_classes', default=cfg.num_classes, help='num of class')
    # ----------------------Required arguments
...
def main():
    ...
    for i, qname in enumerate(queryset_names):
        if qname.find("-") > -1:
            qname = qname.split("-")[0]
        if qname not in valid_datasets:
    ...
    sm_set = ['membership_attack,shadow_model,adversarial_deepfool,vmi'] #select_algorithm_1,if (mi): mi_algorithm, select_algorithm_2
                                                                         #['adversarial_deepfool,,'] #select_algorithm,,
    for sm_m in sm_set:
        start_time = []
        start_time.append(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))#1
        exp = sm_m.split(',')
        ...
        if params['sampling_method'] == 'label':
            ...
            # query probability vectors
            # resume the queried samples; load preset initial seeds
            initial_seed = osp.join(cfg.transfer_set_out_dir, cfg.queryset,
                                   "initial_idxset_{}.npy".format(cfg.DEFAULT_SEED))
            # if start from a interruption: file is "{}.npy".format(sm_m)
            # if start from the first step: file is "initial_idxset_{}.npy".format(cfg.DEFAULT_SEED)
        ...
        start_time.append(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))#this means the end of read initial seed #2

        # ----------- Set up attack model
        attack_model_name = params['model_arch']
        pretrained = params['pretrained']

        # ----------- shadow_model training/loading
        criterion_train = model_utils.soft_cross_entropy
        if (cfg.imp_vic_mem and cfg.vic_mem_method =='shadow_model') \
                or params['second_sss'] == 'shadow_model' \
                or (params['sampling_method'] == 'membership_attack' and params['ma_method'] == 'shadow_model'):
            # # 取10000，10000
            ...
            if cfg.read_shadow_from_path:
                ...
                if cfg.transfer_mi:
                    ...
                    p_save['created_on'] = str(datetime.now())
            ...
            # Here test
            if cfg.mi_test:
                y_mi_test, y_mi_target = s_adversary.get_transferset_shadow_test(k=len(s_queryset))
                confidence = []
        ...
        start_time.append(datetime.now().strftime('%Y-%m-%d %H:%M:%S')) #3
        ...
        if params['iterative']:  # start iteration train
            for it in range(params['num_iter']):
                ...
                # ----- Train
                it_time.append(datetime.now().strftime('%Y-%m-%d %H:%M:%S')) #4
                ...
                if False: #it==0
                    ...
                    model_utils.train_model(model=attack_model, trainset=transferset, out_path=out_path,
                                            ...
                                            optimizer=optimizer,
                                            s_m=sm_m, args=model_args, imp_vic_mem=cfg.imp_vic_mem, **params)
                    # Store arguments
                ...
                it_time.append(datetime.now().strftime('%Y-%m-%d %H:%M:%S')) #5
                ...
                if (it + 1) < params['num_iter']:
                    ...
                    if params['sampling_method'] == 'membership_attack':
                    ...
                    print('=> Training at budget = {}'.format(len(transferset)))
                    # update(shadow_model)
                    it_time.append(datetime.now().strftime('%Y-%m-%d %H:%M:%S')) #6
                    ...
                    it_time.append(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))  #7
