...
def train_attack_model(X_train, y_train, n_hidden=50, batch_size=100, epochs=100, learning_rate=0.01, model='cnn', l2_ratio=1e-7, shadow_model_path='data'):
    train_x = X_train
    train_y = y_train
    n_in = train_x.shape
    print("n_in", n_in) #(5000,10)

    n_out = len(np.unique(train_y)) #0,1
    ...
    # use trained network for predictions
    test_prediction = lasagne.layers.get_output(output_layer, deterministic=True)
    test_fn = theano.function([input_var], test_prediction) #用了theano
    print('Training...')
    # counter = 1
    ...
    for epoch in range(epochs):
        loss = 0
        for input_batch, target_batch in iterate_minibatches(train_x, train_y, batch_size):
            loss += train_fn(input_batch, target_batch) #X, y
        ...
        if (epoch % 10 == 0):
            ...
            for input_batch, _ in iterate_minibatches(train_x, train_y, batch_size, shuffle=False):
                ...
                pred_y.append(np.argmax(pred, axis=1))
            pred_y = np.concatenate(pred_y)
            acc = np.sum(pred_y == train_y).astype(int)
            # print("pred_y==train_y:", (pred_y == train_y).shape)
            print('bi_classifier [training] acc:', acc, '/', len(pred_y))
        # counter = counter + 1
    ...
    #     for input_batch, _ in iterate_minibatches(test_x, test_y, batch_size, shuffle=False):
    #         # input_batch = (np.reshape(input_batch,(len(input_batch),3,32,32)))
    #         pred = test_fn(input_batch) #output
    #         pred_y.append(np.argmax(pred, axis=1))
    #     pred_y = np.concatenate(pred_y)
    #     print('Testing Accuracy: {}'.format(accuracy_score(test_y, pred_y)))
    #     # print("predy:\n", pred_y)
    ...
    # return output_layer
    # 一个np.array, [0, 1, 0, 1, 1...]#两百个

    # 保存模型
    theano_path = osp.join(shadow_model_path, 'classfier_model.pickle')
    with open(theano_path, 'wb') as file:
        pickle.dump(test_fn, file)

    return test_fn


def load_attack_model(X_train, y_train, n_hidden=50, batch_size=100, epochs=100, learning_rate=0.01, model='cnn',
                       l2_ratio=1e-7, shadow_model_path='data'):
    train_x = X_train
    train_y = y_train
    n_in = train_x.shape
    print("n_in", n_in)  # (5000,10)
    ...
    theano_path = osp.join(shadow_model_path, 'classfier_model.pickle')
    # 提取模型
    with open(theano_path, 'rb')as file:
        test_fn = pickle.load(file)
